...
public class Thread02Main {
	...
	public static void main(String[] args) {
		...
		// 3. Runnable을 구현하는 클래스의 인스턴스를 생성
		Runnable runnable1 = new MyRunnable("유튭친구들 아뇽~~~~");
		Runnable runnable2 = new MyRunnable("김우경 바보시끼");
		Runnable runnable3 = new MyRunnable("아ㅣ이 배고파 ㅠㅠ");
		...
		new Thread(new MyRunnable("짜장면 or 타코")).start();
// 스레드가 돌아가고있으면 프로그램이 끝난것이 아니다. (메인문이 끈난다고 프로그램이 끈나는 것이 아니다. )		
		
		
		// Anonymous class 로 생성 // 익명클래스
		//System.out.println();
		//System.out.println("Anonymous class 으로 Runnable 구현");
		new Thread(new Runnable() {
			int count = 0;
			String msg = "밥머그러 가즈아아아";
			@Override
			public void run() {
				for (int i = 0; i < 100; i++) {
		...
		new Thread(()->{
			for (int i = 0; i < 100; i++) {
				//Thread.currentThread() - 현재 이 코드를 실행중인 Thread 객체 리턴
				System.out.println(Thread.currentThread().getName() +" "+i + ":" +"쉬는시간이당");
