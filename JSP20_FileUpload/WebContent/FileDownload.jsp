...
//파일을 다운로드 할때 파일의 제목이 인코딩에 따라 제목이 깨질수 있다.
	...
	System.out.println("파일유형(MINE) : "+fileType);
	
	
	//3. response 세팅
	//response 에 유형 (MINE)을 알려준다. 
	
	response.setContentType(fileType);
	
	//원본파일 이름으로 다운로드 받을 수 있게 처리 
	...
	out.clear(); //out객체 내 출력할 내용들 제거.
	out = pageContext.pushBody();//<=커스텀 태그등도 clear 하고 JspWriter(out)에 새로운 body 생성
	...
	System.out.println("전송 byte : "+totalRead+"byte");
