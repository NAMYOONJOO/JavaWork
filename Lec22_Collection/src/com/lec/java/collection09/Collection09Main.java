...
public class Collection09Main {
	...
	public static void main(String[] args) {
		...
		// enhanced for
		System.out.println();
		System.out.println("enhanced for"); // 오름차순으로 나옴
		for (Integer e : tset) {
		...
		System.out.println();
		TreeSet<Integer> subSet = (TreeSet<Integer>) tset.subSet(5, 14); // 형변환 해야한다. //14전까지 !
		System.out.println(subSet);// toString이 오버라이딩 되어있다********

		subSet = (TreeSet<Integer>) tset.subSet(5,true, 14,true); // 5포함 14포함
		System.out.println(subSet);// toString이 오버라이딩 되어있다********

		subSet = (TreeSet<Integer>) tset.subSet(3, 10); //범위 정할 때 꼭 있는데이터로 범위를 정하지 않아도 된다. 
